...
class ProbabilityPlayer(BasePokerPlayer):
  ...
  def declare_action(self, valid_actions, hole_card, round_state):
    ...
    wining_probability = estimate_hole_card_win_rate(300, 2, gen_cards(hole_card), gen_cards(round_state['community_card']));
    # the mathmatical expectation of Fold is 0
    # ex_call = wining_probability*(round_state[''] + moneyToCall) - moneyToCall;
    # ex_raise = wining_probability*(moneyOnTable + 2*moneyToRaise) - moneyToRaise;
    ...
    if wining_probability > 0.5 and has_action_raise == True:        
        action = 'raise'
